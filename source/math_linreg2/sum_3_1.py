...
# results_PLS = perform_cv(PLSRegression(), 'n_components', [1, 2, 3, 4, 5, 6, 7, 8])
...
# plot_results(ax3, results_PLS, 'param_n_components', 'pls')

""" ##############################################################
###############################################"""

# def perform_pca_and_sort_by_tvalues(X):
#     if_passed = False
...
#     X_pca_sorted = X_pca[:, np.argsort(-tvalues_square)]
#     return X_pca_sorted
...
# tvalues_square = tvalues * tvalues
# X_pca_sorted = X_pca[:, np.argsort(-tvalues_square)]
# X_pca_sorted = perform_pca_and_sort_by_tvalues(X_scaled)
indices = perform_pca_and_sort_by_tvalues(X_scaled)


class MyTemplateEstimator(BaseEstimator):
    def __init__(self, n_components=1):
        self.n_components = n_components

    def fit(self, X, y):
            self.linreg = LinearRegression()
            self.linreg.fit(perform_pca_and_sort_by_tvalues(X)[:, 0:self.n_components], y)
            return self

    def predict(self, X):
            return self.linreg.predict(perform_pca_and_sort_by_tvalues(X)[:, 0:self.n_components])


class MyTemplateTransformer(BaseEstimator, TransformerMixin):
    def __init__(self, n_components=1):
        self.n_components = n_components
    ...
    def transform(self, X):
        global indices
        # return perform_pca_and_sort_by_tvalues(X)[:, 0:self.n_components]
        return X[:, indices][:, 0:self.n_components]


test1 = MyTemplateTransformer(n_components=8)
test1.fit(X_scaled)
z1 = test1.transform(X_scaled)



# sth1 = MyTemplateEstimator(n_components=3)
# sth1.fit(X_scaled, y_scaled)
# sth1.predict(X_scaled)


cv = KFold(n_splits=10)
scorer = sklearn.metrics.make_scorer(sklearn.metrics.mean_squared_error)
grid = GridSearchCV(Pipeline(steps=[('pca', MyTemplateTransformer()), ('linreg', LinearRegression())]), scoring=scorer, param_grid={'pca__n_components': [1, 2, 3, 4, 5, 6, 7, 8]}, cv=cv)
grid.fit(X_scaled, y_scaled)
...
perform_cv(Pipeline(steps=[('pca', MyTemplateTransformer()), ('linreg', LinearRegression())]), 'pca__n_components', [1, 2, 3, 4, 5, 6, 7, 8])
...
""" test srotowania """
# list(np.var(X_pca, axis=0))
# list(tvalues_square)
# list(np.var(X_pca_sorted, axis=0))