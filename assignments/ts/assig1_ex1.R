...
m1  <- lm(r3~r1)
summary(m1)
conclusion1 <- "One can say: linear relationships between series seems to be 
significant (low p-values). However, it depends on whether the asymtotic 
normality assumptions are met. We need to check residuals."
cat(conclusion1)
...
summary(m2)
conclusion3 <- "Linear relationship between 'c1' and 'c3' seems to be significant
(low p-values).
"
cat(conclusion3)
...
conclusion4 <- "Acf plot shows us, that residuals in c3~c1 are, in general, uncorrelated.
...
m3
conclusion5 <- "The second thing is that MA(1) (with regressor) seems to be equally good model 
compared time-agnostic model (they both output same coefficient and R-squares).
This strengthens our conviction that relationship between 'c1' and 'c3' is significant.
"
cat(conclusion5)
...
rsq
conclusion6 <- "R square is high. It enables accurate predictions to be made."
cat(conclusion6)
